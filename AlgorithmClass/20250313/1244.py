...
def solve(cnt):  # 최대 상금을 출력하는 함수
    ...
    if cnt == N:  # 교환 횟수를 모두 썼다면
        result = lst[cnt - 1].pop()  # 재귀 호출 결과를 pop 하여 저장.
        prize = 0  # 상금을 계산하기 위해 변수를 설정.
        for ii in range(len(result)):
            prize += result[ii] * 10 ** (L - ii - 1)  # 상금을 계산
        if answer < prize:  # 최대 상금보다 더 많다면
            answer = prize  # 최대 상금 갱신.
        return  # 이후 함수 종료.
    ...
    for i in range(L - 1):  # 첫 번째 카드를 고름.
        for j in range(i + 1, L):  # 두 번째 카드를 고름.
